// This function renders all the elements in the html of the browser before running the code
...
let now = dayjs();
//  Returns the hour (9 to 17) of a date.
...
const setColor = function (hour) {
  ...
  // Compares the id to the current hour and applies corresponding attributes.
  if (hour < curHour) {
...
// This function gets user input that is saved in localStorage.
const loadData = function (hour) {
  let hourEl = "#hour-" + hour;
  const textEl = $(hourEl).children("textarea")[0];
  textEl.value = localStorage.getItem("hour-" + hour);
};
// This function sets the value of corresponding text areas.
const save = function () {
